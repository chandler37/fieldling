...
/**
 */
public class TimeSlaveHelper {
    
    Vector slaves = new Vector();
    int lastTimeBroadcast = -1;
    
    public TimeSlaveHelper() {}
    
    public void addSlave(TimeSlave slave) {
        if (slave == null)
            throw new NullPointerException();
        if (!slaves.contains(slave))
            slaves.addElement(slave);
    }
    public void removeSlave(TimeSlave slave) {
        slaves.remove(slave);
    }
    
    public synchronized void broadcastTimeChanged(int time, TimeSlave source) {
        if (time != lastTimeBroadcast) {   
          lastTimeBroadcast = time; // set this before notifying any slaves,
                                    // as each slave will recursively call this
                                    // method
            for (int i=0, n=slaves.size(); i<n; i++) {
                TimeSlave slave = (TimeSlave) slaves.elementAt(i);
                if (slave != source) 
                    slave.timeChanged(time);
            }
        }
    }
}
