...
public class QD extends JDesktopPane implements DOMErrorHandler {
	...
	{
		if (configuration.getParameters().get("qd.title")!=null)
		{
			String transcriptTitle;
			transcriptTitle = XPathUtilities.saxonSelectSingleDOMNodeToString(getEditor().getXMLDocument(), (XPathExpression)(configuration.getParameters().get("qd.title")));
			if (transcriptTitle!=null && !transcriptTitle.equals(""))
				return transcriptTitle + " - " + QD.productName;
		}
