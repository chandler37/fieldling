...
public class SetPreferences extends BasicTask {
	...
	class InterfacePreferenceSetter implements PreferenceSetter {
		...
		String old_fontFace, old_interfaceFontFace;
		JComboBox romanFontFamilies;
		int old_fontSize, old_defaultLanguage;
		JComboBox romanFontSizes;
		private JComboBox defaultLanguage = null, interfaceFontFamilies = null;
		...
		private void updateSupportedFonts()
		{
			...
			interfaceFontFamilies.setModel(model);
			String defaultFont = old_interfaceFontFace; 
			/* if there is no default language font set for Quilldriver,
			 * use the system default font.
			 */ 
			if (defaultFont==null) defaultFont = ((Font)UIManager.get("Label.font")).getFamily();
			for (i=0; i<fontNames.length; i++)
				if (fontNames[i].equals(defaultFont))
				{
					interfaceFontFamilies.setSelectedIndex(i);
					break;
				}
			if (i>=fontNames.length)
				interfaceFontFamilies.setSelectedIndex(0);
		}
		...
		public InterfacePreferenceSetter(QD qd) {
			...
			old_interfaceFontFace = PreferenceManager.getValue(PreferenceManager.DEFAULT_INTERFACE_FONT_KEY, PreferenceManager.DEFAULT_INTERFACE_FONT_DEFAULT);
			/* Combo is not filled yet, as the supported fonts
			 * depend on the language selected.
			 */
			interfaceFontFamilies = new JComboBox();
			
			old_defaultLanguage = PreferenceManager.getInt(PreferenceManager.DEFAULT_LANGUAGE_KEY, PreferenceManager.DEFAULT_LANGUAGE_DEFAULT); 
			 ...
			 * default language.
			 */
			if (old_defaultLanguage>-1)
			{
				defaultLanguage.setSelectedIndex(old_defaultLanguage);
			}
			else
			{
			...
			languagePanel.add(new JLabel(QD.messages.getString("Language")));
			languagePanel.add(defaultLanguage);
			
			JPanel interfaceFontPanel = new JPanel(new FlowLayout(FlowLayout.LEFT));
			interfaceFontPanel.add(new JLabel(QD.messages.getString("InterfaceFont")));
			interfaceFontPanel.add(interfaceFontFamilies);
			...
			@TIBETAN@fontAndSizeLabel = QD.messages.getString("NonTibetanTranscriptFontAndSize");
			@UNICODE@fontAndSizeLabel = QD.messages.getString("TranscriptFontAndSize");
			...
			@UNICODE@preferencesPanel.setLayout(new GridLayout(4,1));
			@TIBETAN@preferencesPanel.setLayout(new GridLayout(5,1));
			preferencesPanel.add(tagColorPanel);
			preferencesPanel.add(languagePanel);
			preferencesPanel.add(interfaceFontPanel);
			@TIBETAN@preferencesPanel.add(tibetanFontSizePanel);
			preferencesPanel.add(fontAndSizePanel);				
		}
		...
		public void setPreferences() {
			boolean reRender = false, restart = false;
			...
			int new_defaultLanguage = defaultLanguage.getSelectedIndex();
			if (old_defaultLanguage != new_defaultLanguage)
			{
				PreferenceManager.setInt(PreferenceManager.DEFAULT_LANGUAGE_KEY, new_defaultLanguage);
				restart = true;
			}
			
			String new_interfaceFontFace = (String)romanFontFamilies.getSelectedItem();
			if (!new_interfaceFontFace.equals(old_interfaceFontFace))
			{
				PreferenceManager.setValue(PreferenceManager.DEFAULT_INTERFACE_FONT_KEY, new_interfaceFontFace);
				restart = true;
			}
			...
			if (restart) {
				JOptionPane.showMessageDialog(qd, QD.messages.getString("ChangesToInterface"));
			}				
		}
	}
}
